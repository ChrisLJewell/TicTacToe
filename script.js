...
function playAgain() {
    ...
    if (playerVsComputerRadio.checked) {
        // Remove event listeners for player vs player mode
        cells.forEach(cell => {
            cell.removeEventListener('click', cellClicked);
        });

        // Add event listeners for player vs AI mode
        cells.forEach(cell => {
            cell.addEventListener('click', playerVsAI);
        });
    } else if (playerVsPlayerRadio.checked) {
        // Remove event listeners for player vs AI mode
        cells.forEach(cell => {
            cell.removeEventListener('click', playerVsAI);
        });

        // Add event listeners for player vs player mode
        cells.forEach(cell => {
...
function resetGame() {
  ...
  }else{ changeH1.innerHTML = "O's turn";
...
let playAgain = document.querySelector('#playAgain').style.display = "none";
